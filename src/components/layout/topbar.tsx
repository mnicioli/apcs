...
export function Topbar({ user, role }: { user: SessionUser; role: Role }) {
  return (
    <header className="border-border bg-background flex h-14 shrink-0 items-center justify-between gap-3 border-b px-4">
      {/*
        A MARCA NO CELULAR. A lateral (que carrega a assinatura completa) é
        `hidden md:flex` — abaixo disso o sistema não exibia identificação
        nenhuma. Aqui vai só o desenho: o nome por extenso ao lado dele não cabe
        numa barra que ainda precisa do nome do usuário e do botão de sair.

        O `div` vazio no lugar dela a partir de `md` mantém o `justify-between`
        empurrando o bloco da direita para a direita.
      */}
      <Link href="/dashboard" className="md:hidden" aria-label="Ir para o painel">
        <ApcsMark height={24} />
      </Link>
